...
export default class ODHSupportStandardDataSummerForm extends Form {
	...
	render() {
		...
		const { technicalOperationsList, odhSupportStandardsList } = this.props;
    ...
    const title = IS_CREATING ? 'Добавление показателя нормы по содержанию ОДХ (лето)' : 'Изменение показателя нормы по содержанию ОДХ (лето)';
		...
		const ODH_STANDARDS = odhSupportStandardsList.map(({id, standard}) => ({value: id, label: standard}));
