...
export function getEverGisToken(): Promise<string> {
  const tokenUrl = `http${/s/.test(window.location.protocol) ? 's' : ''}://gisoiv.mos.ru/IntegrationGIS/SpatialProcessor/Strategis.JsClient/ApiLogin.aspx?authId=505741D8-C667-440D-9CA0-32FD1FF6AF88&userName=ets_int&password=ejm*xbYz0G&ts=${new Date().getTime()}`;
