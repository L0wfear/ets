...
export default class WaybillInProgress extends WaybillClosed {
  ...
  renderSubitems(subItems) {
    ...
    si = si.map((ar) => {
      const array = cloneDeep(ar);

      array[0].data.groupStart = true;
      array[array.length - 1].data.groupEnd = true;
      return array;
    });
    si = flatten(si);
    subItems = si;
      ...
      <ul>
        {subItems.map((item, i) => (<div key={i}>
          ...
          <li key={i} onClick={this.action.bind(this, item)}>
            {`№${item.data.waybill_number}, `}<span style={{ fontWeight: 'bold' }}>{item.data.car_gov_number}</span>, {item.data.car_garage_number ? item.data.car_garage_number : '-'}
            <br />
  ...
  renderItems() {
    const canView = this.context.flux.getStore('session').getPermission(['waybill.read', 'waybill.departure_and_arrival_values'], true);
