...
export const dashboardSetInfoDataInWaybillCompleted = (infoData = null, activeIndex = 0) => ({
  type: DASHBOARD_SET_INFO_DATA,
  payload: {
...
export const dashboardLoadCardData = (path, payload = {}) => ({
  ...
  payload: (defAns.backEndKeys[path] || path).split('/').reduce((ApiService, path) => ApiService.path(path), DashboardService)
    .get(payload)
    .catch(e => {
      console.warn(e);
