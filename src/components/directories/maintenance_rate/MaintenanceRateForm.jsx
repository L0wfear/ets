...
export default class MaintenanceRateForm extends Form {
  ...
  render() {
    ...
    const { isPermitted = false, technicalOperationsList, type, maintenanceWorkList, cleanCategoriesList } = this.props;
    const TECH_OPERATIONS = technicalOperationsList.map(({ id, name }) => ({ value: id, label: name }));
    const MAINTENANCE_WORK = maintenanceWorkList.map(({ id, name }) => ({ value: id, label: name }));
          ...
          <Row>
            <Col md={12}>
              <Field
                ...
                onChange={this.handleChange.bind(this, 'maintenance_work_id')}
                disabled={!isPermitted}
              />
            </Col>
          </Row>
