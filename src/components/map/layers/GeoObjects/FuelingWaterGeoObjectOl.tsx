...
import { getFuelingWaterGeoObjects, getSelectedType } from 'redux/selectors/geoObjects';
import { GEOOBJECTS_TYPES } from 'constants/geoobjects.js';
import { selectGeometry } from 'redux/modules/geoObjects.js';

@connect(
  state => ({
    geoObjects: getFuelingWaterGeoObjects(state),
    fuelingWaterIsSelected: getSelectedType(state) === GEOOBJECTS_TYPES.fueling_water,
  }),
  {
    selectGeometry,
  },
)
class FuelingWaterLayer extends OlLayer {
  componentDidMount() {
    this.addLayer();
  }
  ...
  withState() {
    return {
      fuelingWaterIsSelected: false,
    };
  }
  inheritComponentWillReceiveProps(props) {
    const { fuelingWaterIsSelected: old_value } = this.props;
    const { fuelingWaterIsSelected: new_value } = props;
  ...
  handleSingleClick = featrue => {
    ...
    this.props.selectGeometry(GEOOBJECTS_TYPES.fueling_water, featrue.get('data'));
...
export default FuelingWaterLayer;
