...
import { getPedestrianTunnelsGeoObjects, getSelectedType } from 'redux/selectors/geoObjects';
import { GEOOBJECTS_TYPES } from 'constants/geoobjects.js';
import { selectGeometry } from 'redux/modules/geoObjects.js';

@connect(
  state => ({
    geoObjects: getPedestrianTunnelsGeoObjects(state),
    pedestrianTunnelsIsSelected: getSelectedType(state) === GEOOBJECTS_TYPES.pedestrian_tunnels,
  }),
  {
    selectGeometry,
  },
)
class PedestrianTunnelsLayer extends OlLayer {
  componentDidMount() {
    this.addLayer();
  }
  ...
  withState() {
    return {
      pedestrianTunnelsIsSelected: false,
    };
  }
  inheritComponentWillReceiveProps(props) {
    const { pedestrianTunnelsIsSelected: old_value } = this.props;
    const { pedestrianTunnelsIsSelected: new_value } = props;
  ...
  handleSingleClick = featrue => {
    ...
    this.props.selectGeometry(GEOOBJECTS_TYPES.pedestrian_tunnels, featrue.get('data'));
...
export default PedestrianTunnelsLayer;
