...
import { getSnowStorageGeoObjects, getSelectedType } from 'redux/selectors/geoObjects';
import { GEOOBJECTS_TYPES } from 'constants/geoobjects.js';
import { selectGeometry } from 'redux/modules/geoObjects.js';

@connect(
  state => ({
    geoObjects: getSnowStorageGeoObjects(state),
    snowStorageIsSelected: getSelectedType(state) === GEOOBJECTS_TYPES.snow_storage,
  }),
  {
    selectGeometry,
  },
)
class SnowStorageLayer extends OlLayer {
  componentDidMount() {
    this.addLayer();
  }
  ...
  withState() {
    return {
      snowStorageIsSelected: false,
    };
  }
  inheritComponentWillReceiveProps(props) {
    const { snowStorageIsSelected: old_value } = this.props;
    const { snowStorageIsSelected: new_value } = props;
  ...
  handleSingleClick = featrue => {
    ...
    this.props.selectGeometry(GEOOBJECTS_TYPES.snow_storage, featrue.get('data'));
...
export default SnowStorageLayer;
