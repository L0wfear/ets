...
import { getSspGeoObjects, getSelectedType } from 'redux/selectors/geoObjects';
import { GEOOBJECTS_TYPES } from 'constants/geoobjects.js';
import { selectGeometry } from 'redux/modules/geoObjects.js';

@connect(
  state => ({
    geoObjects: getSspGeoObjects(state),
    sspIsSelected: getSelectedType(state) === GEOOBJECTS_TYPES.ssp,
  }),
  {
    selectGeometry,
  },
)
class SspLayer extends OlLayer {
  componentDidMount() {
    this.addLayer();
  }
  ...
  inheritComponentWillReceiveProps(props) {
    const { sspIsSelected: old_value } = this.props;
    const { sspIsSelected: new_value } = props;
  ...
  handleSingleClick = featrue => {
    ...
    this.props.selectGeometry(GEOOBJECTS_TYPES.ssp, featrue.get('data'));
