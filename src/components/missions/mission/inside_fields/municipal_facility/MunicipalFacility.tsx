...
type GetMunicipalFacilityFunc = (
  props: GetMunicipalFacilityFuncProps,
) => Promise<GetMunicipalFacilityFuncAns>;
...
class MunicipalFacilityField extends React.PureComponent<
...
> {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (triggerOnUpdate) {
      const toData = technicalOperationsList.find(
        ({ id }) => id === technical_operation_id,
      );

      if (toData) {
        const { normatives } = toData;
  ...
  loadMunicipalFacility(outerPayload) {
    this.props
      ...
      .then(({ municipal_facility_list }) => {
        const { value, typeIdWraomWaybill } = this.props;

        const MUNICIPAL_FACILITY_OPTIONS = municipal_facility_list.reduce(
          (newArr, mfData) => {
            const carAddToArr =
              !typeIdWraomWaybill ||
              (typeIdWraomWaybill &&
                mfData.car_func_types.some(
                  (mfDataCarFuncType) =>
                    mfDataCarFuncType.id === typeIdWraomWaybill,
                ));
        ...
        if (value) {
          const mfOption = MUNICIPAL_FACILITY_OPTIONS.find(
            (mfOptionData) => mfOptionData.value === value,
          );
          if (mfOption) {
            this.props.getDataByNormatives(mfOption.mfData.normatives);
          } else {
            // если нет norm_id
  ...
  render() {
    const { error, disabled, value, clearable, modalKey } = this.props;
