...
import {
  getActiveClassName,
  getClassNameByType,
} from 'components/monitor/new/tool-bar/car-data/car-legend-status/utils';
import {
  listStatus,
} from 'components/monitor/new/tool-bar/car-data/car-legend-status/const';

import {
  PropsCarLegendStatus
} from 'components/monitor/new/tool-bar/car-data/car-legend-status/CarLegendStatus.h';
    ...
    <div className="tool_bar-block">
      <div className="default_cube">
        <div className={getActiveClassName(props)} onClick={props.toggleActiveStatus}>
          <div>{`Активно: ${listStatus.slice(0,3).reduce((summ, { key }) => summ + props[key], 0)}`}</div>
        </div>
