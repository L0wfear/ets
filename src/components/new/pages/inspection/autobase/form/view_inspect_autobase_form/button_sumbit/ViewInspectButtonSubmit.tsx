...
type ViewInspectButtonSubmitProps = {
  ...
  handleSubmit: () => any;
  handleCloseAndAutobaseAct: () => any;
  handleGetAutobaseAct: () => any;
  canSave: boolean;
};
...
export const ViewInspectButtonSubmit: React.FC<ViewInspectButtonSubmitProps> = (props) => {
  ...
  if (props.type === INSPECT_AUTOBASE_TYPE_FORM.close) {
    return (
      <Button disabled={!props.canSave} onClick={props.handleCloseAndAutobaseAct}>Завершить проверку и сформировать акт</Button>
    );
  }

  if (props.type === INSPECT_AUTOBASE_TYPE_FORM.closed) {
    return (
      <Button disabled={!props.canSave} onClick={props.handleGetAutobaseAct}>Cформировать акт</Button>
    );
  }
