...
import { isNotNull, isEmpty, isMotoHoursMileageType, getCarMileageTypeId, parseFloatWithFixed } from 'utils/functions';
...
import { gasDefaultElement, electricalDefaultElement, defaultRefillObj, fuelDefaultElement, defaultMotoHoursData, defaultOdometrData } from 'components/new/pages/waybill/form/context/utils';
...
const getCarMileageTypeIdByStatus = (stateMileageTypeId: Waybill['mileage_type_id'], status: Waybill['status'], carMileageTypeId: Waybill['mileage_type_id']) => {
  if (
    (status === 'active'
    || status === 'deleted'
    || status === 'closed')
    ...
    return stateMileageTypeId;
  }
  return carMileageTypeId;
};
...
export type WaybillState = {
  ...
  electricalOperations: Array<any>;
  equipmentOperations: Array<any>;
  fuelRates: Array<any>;
  gasFuelRates: Array<any>;
  electricalFuelRates: Array<any>;
...
class WaybillForm extends React.Component<WaybillProps, WaybillState> {
      ...
      Promise.all([
        ...
        .then(
          ([
            ...
            const electricalOperations = fuelRates.reduce(
              (
                newArr,
                {
                  id,
                ...
                if (fuelOperationsListById[operation_id] && engine_kind_id === ELECTRICAL_ENGINE_TYPE_ID) {
                  newArr.push({
                    ...fuelOperationsListById[operation_id],
                    name: operation_name,
                    uniqKey: id,
            ...
            const electricalFuelRates = fuelRates.filter(
              ({ engine_kind_id }) => engine_kind_id === ELECTRICAL_ENGINE_TYPE_ID
            );
  ...
  getDutSensors = async (formState, withTs: boolean, withoutPreloader = true) => {
    ...
    if (formState.car_id) {
      ...
      await this.props
        .dispatch(actionGetTrackSensor(
          {
            car_id: formState.car_id,
            ts: withTs
              ? makeUnixTimeMskTimezone(formState.fact_arrival_date)
              : '' 
          }, {
            withoutPreloader,
  ...
  getFieldsToChangeBasedOnLastWaybill = (lastCarUsedWaybill) => {
    ...
    if (isNotNull(lastCarUsedWaybill)) {
      ...
      if (isNotNull(lastCarUsedWaybill.electrical_fuel_type)) {
        fieldsToChange.electrical_fuel_type = lastCarUsedWaybill.electrical_fuel_type;
      }
      ...
      return checkMissionSelectBeforeClose(
        ...
        .then(async () => {
          if (this.props.formState.status === 'active' || this.props.formState.status === 'deleted') {
            ...
            await this.getDutSensors(this.props.formState, true, false);
  ...
  handleChangeElectricalTaxes = (electrical_tax_data, field = 'electrical_tax_data', index = null) => {
    this.handleChange(
      field,
      isArray(electrical_tax_data)
        ? [...electrical_tax_data]
        : electrical_tax_data,
      index
    );
  ...
  render() {
    ...
    const IS_KAMAZ = (get(carIndex, `${state.car_id}.model_name`) || '')
      .toLowerCase()
      .includes('камаз');
    const CAR_HAS_ODOMETER = state?.mileage_type_id
      ? !isMotoHoursMileageType(state.mileage_type_id)
      : null;
    const DRIVERS
    ...
    const activeTrailer = TRAILERS.find((option) => option.value === state.trailer_id);
    const activeTrailerLabel = get(activeTrailer, 'label', null);
        ...
        <ModalBodyPreloader
                  ...
                  <ExtField
                    ...
                    value={
                      state.trailer_id && !(IS_ACTIVE && activeTrailerLabel && isTrailerRequired && state.trailer_id)
                        ...
                        : IS_ACTIVE && isUsePouringMission && isTrailerRequired ? activeTrailerLabel
                          : activeTrailerLabel ? activeTrailerLabel
                            : 'Н/Д'
                    }
                  />
                </EtsBootstrap.Col>
          ...
          {Boolean(state.car_id) && (
            <EtsBootstrap.Row>
              ...
              {state.equipment_fuel && !isElectricalKind && (
                <EtsBootstrap.Col md={4}>
                  <ExtField
                    id="is_one_fuel_tank"
                    type="select"
                    ...
                    <EtsBootstrap.Col md={12}>
                      <EtsBootstrap.Col md={12}>
                        <FuelNavHeader
                          ...
                          isElectricalKind={isElectricalKind}
                        />
                      </EtsBootstrap.Col>
                    </EtsBootstrap.Col>
                    <EtsBootstrap.Col md={12}>
                      ...
                      {/* <-- end  Tab gas */}
                      {/* <-- start  Tab electrical */}
                      {
                        <ElectricalBodyContainer
                          modalKey={modalKey}
                          waybillState={this.state}
                          waybillFormState={state}
                          use_pouring={usePouring && !this.state.isGasKind}
                          ...
                          origFormState={origFormState}
                          handleChange={this.handleChange}
                          handleChangeElectricalReFill={this.handleChangeElectricalReFill}
                          page={this.props.page}
                          path={this.props.path}
                          CAR_HAS_ODOMETER={CAR_HAS_ODOMETER}
                          setTotalValueError={this.props.setTotalValueError}
                          electricalFuelCardsList={this.props.electricalFuelCardsList}
                          electrical_tax_data={electrical_tax_data}
                          tax_data={tax_data}
                          FUEL_TYPES={FUEL_TYPES}
                          IS_KAMAZ={IS_KAMAZ}
                          disableFieldWaybillCarRefill={disableFieldWaybillCarRefill}
                          handleChangeTaxes={this.handleChangeElectricalTaxes}
                          isFuelKind={this.state.isFuelKind}
                          isElectricalKind={isElectricalKind}
                          showComponent={this.state.fuelActiveTabKey === 'electrical'}
                          handleEquipmentFuel={this.handleEquipmentFuel}
                          updateEngineKindsFields={this.updateEngineKindsFields}
                          isGasKind={this.state.isGasKind}
                        />
                        ...
                        <EtsBootstrap.Col md={12} zIndex={1}>
                          <EtsBootstrap.Col md={12}>
                            <EquipmentTaxes
                              ...
                              title={`Расчет ${!isElectricalKind ? 'топлива' : 'ЭЭ'} по норме для оборудования`}
                              noDataMessage={`Для данного ТС нормы расхода ${!isElectricalKind ? 'топлива' : 'ЭЭ'} для спецоборудования не указаны.`}
          ...
          <EtsBootstrap.Row>
            ...
            <EtsBootstrap.Col md={4} style={{marginTop: '14px',}}>
              <ExtField
                type="number"
                label={`Общий расход ${!isElectricalKind ? 'топлива, л' : 'ЭЭ, кВт'}`}
                value={taxesTotal}
