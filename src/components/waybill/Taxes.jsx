...
export default class Taxes extends React.Component {
    ...
    this.tableCellRenderers = {
      OPERATION: (OPERATION, row, index) => {
        if (props.readOnly) {
          ...
          return operation ? `${operation.name} ${row.comment ? `(${row.comment})` : ''}${row.is_excluding_mileage ? ' [без учета пробега]' : ''}` || '' : '';
        }
  ...
  static getDerivedStateFromProps(nexProps, prevProps) {
    ...
    operations = operations.map(data => ({
      ...
      name: data.name,
      label: `${data.comment ? `${data.name} (${data.comment})` : data.name}${data.is_excluding_mileage ? ' [без учета пробега]' : ''}`,
