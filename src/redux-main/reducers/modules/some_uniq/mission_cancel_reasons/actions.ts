...
export const actionSetMissionCancelReasons = (missionCancelReasonsList: IStateSomeUniq['missionCancelReasonsList']) => (dispatch) => (
  dispatch(
    someUniqSetNewData({
...
export const actionGetAndSetInStoreMissionCancelReasons: any = (payload = {}, { page, path }: { page?: string; path?: string }) => async (dispatch) => {
  const { payload: { data } } = await dispatch(
    actionGetCompanyStructure(payload, { page, path }),
  );

  dispatch(
    actionSetMissionCancelReasons(data),
  );
